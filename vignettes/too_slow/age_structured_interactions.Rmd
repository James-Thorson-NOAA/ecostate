...
# Install package for vignette (not on CRAN)
install.packages("remotes")
remotes::install_github("james-thorson/lanczosRTMB")
...
Alternatively, we could fit the model using a custom-build optimizer, although it is memory expensive so we disable it here:

```{r, eval = FALSE}
# Extract elements
tape = GetTape(out$obj)
tape$simplify(); tape$reorder
gr = tape$jacfun()
gr$simplify(); gr$reorder
Hq = make_Hq( 
  tape, 
  x0 = out$obj$par,
  method = "sparse"
...
If useful, we can also approximate the marginal likelihood.  However, it is again to memory intensive for this demo:
